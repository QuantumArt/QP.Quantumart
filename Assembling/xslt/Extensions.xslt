...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="LinqHelper">
    ...
    <xsl:choose>
      <xsl:when test="@isPartial = 'false'">public</xsl:when><xsl:otherwise>internal</xsl:otherwise>
    </xsl:choose>
    static partial class LinqHelper
    {
  ...
  <xsl:template name="DataContext">
    ...
    <xsl:choose>
      <xsl:when test="@dbIndependent = 'true'">
        ...
        return XmlMappingSource.FromXml(dbc.GetDefaultMapFileContents(dbc.GetSiteId(DefaultSiteName), nameof(
    ...
    <xsl:variable name="siteUrlVariable">
      <xsl:choose>
        <xsl:when test="@dbIndependent = 'true'">
          (Cnn.IsStage) ? <xsl:value-of select="$stageSiteUrlVariable" /> :
          <xsl:value-of select="$liveSiteUrlVariable" />
        </xsl:when>
        <xsl:when test="@forStage = 'true' ">
          <xsl:value-of select="$stageSiteUrlVariable" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$liveSiteUrlVariable" />
        </xsl:otherwise>
  ...
  <xsl:template match="content">
    ...
    <xsl:apply-templates
      select="parent::node()/content/attribute[@type='O2M' and @has_m2o='false' and @related_content_id=$id]"
      mode="DetachImplicitBackAssociation" />
    ...
    {
    <xsl:if test="parent::node()/@replaceUrls = 'true'">
      <xsl:apply-templates select="attribute[@type = 'String' or @type = 'VisualEdit' or @type = 'Textbox']" />
    </xsl:if>
    }
    <xsl:apply-templates select="attribute[@type = 'Image' or @type = 'File' or @type = 'Dynamic Image']" mode="Files" />
    <xsl:apply-templates
      select="attribute[@type = 'Numeric' or @type = 'Boolean' or @type = 'DateTime' or @type = 'Date' or @type = 'Time']"
      mode="ValueTypes" />
    }
  ...
  <xsl:template match="attribute" mode="Files">
    ...
    <xsl:variable name="attribute_id">
      <xsl:choose>
        <xsl:when test="parent::node()/parent::node()/@dbIndependent = 'true'">
          InternalDataContext.Cnn.GetAttributeIdByNetNames(InternalDataContext.SiteId, "
          <xsl:value-of select="parent::node()/@mapped_name" />", "<xsl:value-of select="@mapped_name" />")
        </xsl:when>
        <xsl:otherwise>
    ...
    _<xsl:value-of select="@mapped_name" />Url = String.Format(@"{0}/{1}", InternalDataContext.Cnn.GetUrlForFileAttribute(
    <xsl:value-of select="$attribute_id" />, true, _InternalDataContext.ShouldRemoveSchema),
    ...
    _<xsl:value-of select="@mapped_name" />UploadPath = InternalDataContext.Cnn.GetDirectoryForFileAttribute(
  ...
  <xsl:template match="attribute" mode="ValueTypes">
    ...
    get
    {
    return (<xsl:value-of select="@mapped_name" />.HasValue) ? <xsl:value-of select="@mapped_name" />.Value : default(
  ...
  <xsl:template match="attribute" xml:space="preserve">
            _<xsl:value-of select="@mapped_name" /> = InternalDataContext.ReplacePlaceholders(_<xsl:value-of
                                                                                                 select="@mapped_name" />);</xsl:template>

  <xsl:template match="content" mode="Filters">
    <xsl:variable name="name" select="@mapped_name" />
    <xsl:variable name="context_name" select="parent::node()/@class" />
    public static IEnumerable&lt;<xsl:value-of select="$name" />&gt; ApplyContext(this IEnumerable&lt;
    <xsl:value-of select="$name" />&gt; e, <xsl:value-of select="$context_name" /> context)
    {
    ...
    public static IQueryable&lt;<xsl:value-of select="$name" />&gt; Published(this IQueryable&lt;
    ...
    <xsl:choose>
      <xsl:when test="@use_default_filtration = 'true'">
        public static IQueryable&lt;<xsl:value-of select="$name" />&gt; ForFrontEnd(this IQueryable&lt;
        <xsl:value-of select="$name" />&gt; e)
        {
        return e;
        }
  ...
  <xsl:template name="GetType">
    ...
    <xsl:choose>
      <xsl:when
        test="$type='String' or $type='Textbox' or $type='VisualEdit' or $type='File' or $type='Image' or $type='Dynamic Image'">
        System.String
      </xsl:when>
  ...
  <xsl:template match="attribute" mode="DetachImplicitBackAssociation">
    ...
    <xsl:if test="$content_plural_name != ''">
      ...
      this._<xsl:value-of select="$back_name" /> = Detach(this._<xsl:value-of select="$back_name" />, attach_
      <xsl:value-of select="$back_name" />, detach_<xsl:value-of select="$back_name" />);
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="attribute" mode="DetachExplicitBackAssociation">
    ...
    <xsl:if test="$related_mapped_name != ''">
      this._<xsl:value-of select="@mapped_name" /> = Detach(this._<xsl:value-of select="@mapped_name" />, attach_
      <xsl:value-of select="@mapped_name" />, detach_<xsl:value-of select="@mapped_name" />);
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="link" mode="DetachLink">
    <xsl:if test="@plural_mapped_name != ''">
      this._<xsl:value-of select="@plural_mapped_name" /> = Detach(this._<xsl:value-of select="@plural_mapped_name" />, attach_
      <xsl:value-of select="@plural_mapped_name" />, detach_<xsl:value-of select="@plural_mapped_name" />);
    </xsl:if>
  </xsl:template>
</xsl:stylesheet>
