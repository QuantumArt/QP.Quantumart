...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="attribute">
    ...
    <xsl:variable name="related_member_name"
                  select="parent::node()/parent::node()/content[@id=$related_content_id]/@plural_mapped_name" />
    <xsl:variable name="related_class_name"
                  select="parent::node()/parent::node()/content[@id=$related_content_id]/@mapped_name" />
    ...
    <xsl:variable name="linked_member_name">
      <xsl:choose>
        <xsl:when test="$related_content_id = $content_id">
          <xsl:value-of select="concat($related_class_name, '2')" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$related_class_name" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="linked_id_member_name">
      <xsl:choose>
        <xsl:when test="$related_content_id = $content_id">
          <xsl:value-of select="concat($related_class_name, '_ID', '2')" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="concat($related_class_name, '_ID')" />
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    ...
    <xsl:variable name="out_link_id">
      <xsl:choose>
        <xsl:when test="$dbIndependent = 'true'">
          InternalDataContext.Cnn.GetLinkIdByNetName(InternalDataContext.SiteId, "
          <xsl:value-of select="$link_class_name" />")
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="$link_id" />
        </xsl:otherwise>
    ...
    <xsl:if test="$related_class_name != ''">
      <xsl:choose>
        <xsl:when test="$virtual = '0' and $related_class_name != ''">
          ...
          private ListSelector&lt;<xsl:value-of select="$link_class_name" />,
          <xsl:value-of select="$related_class_name" />&gt; _<xsl:value-of select="@mapped_name" /> = null;

          public ListSelector&lt;<xsl:value-of select="$link_class_name" />,
          <xsl:value-of select="$related_class_name" />&gt; <xsl:value-of select="@mapped_name" />
          {
          get
          {
          ...
          delegate(IList&lt;<xsl:value-of select="$link_class_name" />&gt; ods,
          <xsl:value-of select="$related_class_name" /> p)
          {
        ...
        <xsl:otherwise>
          ...
          <xsl:if test="$dbIndependent = 'true'">
            if (linkId == 0)
            throw new Exception(String.Format("Junction class '{0}' is not found on the site (ID = {1})", "
            <xsl:value-of select="$link_class_name" />", InternalDataContext.SiteId));
          </xsl:if>
          return InternalDataContext.ExecuteQuery&lt;<xsl:value-of select="$related_class_name" />&gt;(String.Format("EXEC sp_executesql N'select c.* from content_{0} c inner join item_link i on c.content_item_id = i.linked_item_id where i.item_id = @itemId and i.link_id = @linkId', N'@linkId NUMERIC, @itemId NUMERIC', @linkId = {2}, @itemId = {1}",
          <xsl:value-of select="$related_content_id" />, Id, linkId)).AsQueryable();
          }
          }
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="link">
    ...
    <xsl:variable name="link_id">
      <xsl:choose>
        <xsl:when test="parent::node()/@dbIndependent = 'true'">
          InternalDataContext.Cnn.GetLinkIdByNetName(InternalDataContext.SiteId, "
    ...
    <xsl:if test="parent::node()/@dbIndependent = 'true'">
      if (linkId == 0)
      throw new Exception(String.Format("Junction class '{0}' is not found on the site (ID = {1})", "
      <xsl:value-of select="$name" />", InternalDataContext.SiteId));
    </xsl:if>
    return linkId;
    }
    }
