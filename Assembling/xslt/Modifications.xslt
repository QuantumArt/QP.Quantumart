...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="content">
    ...
    <xsl:if test="@virtual = '0'">
      private Dictionary&lt;string,string&gt; Pack<xsl:value-of select="$name" />(<xsl:value-of select="$name" />
      instance)
      {
    ...
    <xsl:variable name="form_name">
      <xsl:choose>
        <xsl:when test="$dbIndependent = 'true'">
          Cnn.GetContentAttributeObject(Cnn.GetAttributeIdByNetNames(SiteId, "
          <xsl:value-of select="$content_name" />", "<xsl:value-of select="@mapped_name" />")).Name
        </xsl:when>
        <xsl:otherwise>"<xsl:value-of select="@name" />"</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    values[<xsl:value-of select="$form_name" />] =
    <xsl:choose>
      <xsl:when test="@type='Boolean'">
        (instance.<xsl:value-of select="$name" />.HasValue) ? (instance.
        <xsl:value-of select="$name" />.Value ? "1" : "0") : null;
      </xsl:when>
      <xsl:when test="@type='O2M' or @type='Numeric' or @type='Date' or @type='Time' or @type='DateTime'">
        (instance.
        <xsl:value-of select="$name" /> != null) ? instance.<xsl:value-of select="$name" />.ToString() : null
      </xsl:when>
